"""
API migration: empty -> 1

10 operations added, 7 schemas added
"""
...
def downgrade_person_out(data: dict) -> dict:
    """1 -> initial: Remove new schema from responses."""
    data.pop("id", None)
    data.pop("name", None)
    data.pop("email", None)
...
def downgrade_pagination_params(data: dict) -> dict:
    """1 -> initial: Remove new schema from responses."""
    data.pop("page", None)
...
data_migrations = DataMigrationSet(
    schema_downgrades=[
        ...
        SchemaDowngrade("#/components/schemas/PaginationParams", downgrade_pagination_params),
    ],
)
